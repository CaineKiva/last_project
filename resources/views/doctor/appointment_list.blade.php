                ...
                @foreach ($array_list as $appointment)
                    <tr>
                        ...
                        <th scope="col" align="center" style="text-align: center;">
                            ...
                            <button type="button" class="btn btn-success fas fa-check" style="color: white;" onclick="update()"
                                    appointment_id = "{{$appointment->appointment_id}}" title="Đã khám xong"></button>
                            ...
                            <button type="button" class="btn btn-info fas fa-exchange-alt" style="color: white;" onclick="change()"
                                    appointment_id = "{{$appointment->appointment_id}}" title="Đổi chuyên khoa/bác sĩ khám"></button>
                        </th>
                    </tr>
                @endforeach
                <br>
        ...
        <div class="card"  >
            ...
            <div class="card-body card-block" >
                <form action="{{ route('appointment.process_update') }}" id="routes" method="post" enctype="multipart/form-data" class="form-horizontal">
                    ...
                    <div class="row form-group" id="medicine_div">
                        <div class="col col-md-3"><label for="select" class=" form-control-label">Kê đơn</label></div>
                        <div class="col-12 col-md-9">
                            <select class="form-control" name="medicine_data[]" id="select_medicine" multiple style="width: 650px">
{{--                                <option selected="selected" value="0" disabled>Thuốc/dược liệu</option>--}}
                    ...
                    <div class="row form-group" id="time_div">
                    ...
                    <div class="row form-group" id="room_div">
        ...
        jQuery(document).ready(function($) {
            ...
            $(document).on('click', '.btn.btn-success.fas.fa-check', function (){
                ...
                $.ajax({
                    ...
                    .done(function(response) {
                        console.log(response);
                        $("strong").text('Dặn dò sau khi khám xong');
                        $("#appointment_id").val(response[0]['appointment_id']);
                        $("#patient_id").val(response[0]['patient_id']);
                        $("#patient_name").val(response[0]['first_name']+' '+response[0]['last_name']);
                        $("#time").val(response[0]['time'].replace(' ', 'T'));
                        ...
                        $("#time_div").hide();
                        $("#medicine_div").show();
                        $("#routes").attr('action','{{ route('appointment.done') }}').attr('method','get');
                    })
            });
